...
import com.cranaya.inc.screens.client.category.list.ClientCategoryListViewModel
...
fun GetCategories(
    ...
    when(val response = viewModel.categoriesResponse) {
        ...
        is Resource.Success -> {
            ClientCategoryListContent(paddingValues = paddingValues, navController = navController, categories = response.data)
        }
        is Resource.Failure -> {
